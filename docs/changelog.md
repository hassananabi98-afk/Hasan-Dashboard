...
- **Balance box height corrected same day.** It was ~15px shorter than the Budget and Card boxes — both have a header line plus a progress-track row, and Savings only had the header. Added a second line (`Added X · Used Y`) and a two-colour track (green share = added, red share = used) in place of a limit-style fill, since there's no ceiling to show progress against. Heights now match exactly. Cache bumped to `?v=142`
- **"Could not add entry" fixed same day — a real bug, not a validation issue.** `authenticated` was never granted `SELECT`/`INSERT`/`UPDATE`/`DELETE` on the table at all; the `owner_only` RLS policy was correct but never got the chance to run, since Postgres denies at the privilege level first. `CREATE TABLE` via raw SQL skips the grants Supabase Studio's own table creator adds automatically — every insert attempt failed with "permission denied for table savings_transactions" in the Postgres logs. Fixed with one `GRANT` statement; logged in `schema.md` as a general note for any future hand-created table
- **Label made optional, same day.** Was required client-side even though the ask never needed a strict label on every entry. Now only the amount gates Save; a blank label stores `''` (the column stays `NOT NULL`) and the log shows "Add"/"Use" in its place. Cache bumped to `?v=143`
- **Displayed log scoped to the salary cycle, same day — balance stays all-time.** The list was showing every entry ever logged and would only ever grow. Now split the same way `card_transactions` already is: the balance (`renderSavingsBox`) still sums the full table, but the visible list (`renderSavingsList`) is filtered through `getPeriodTxns` to the month being viewed, and re-renders on Finance's month navigation. An entry from an earlier cycle — like a starting-balance seed dated when tracking began — counts toward the balance but only shows in the log when that month is the one in view, same as an old card charge does. Cache bumped to `?v=144`
- **Added/used split scoped to cycle too, same day.** The track bar and the "Added X · Used Y" line under it were still all-time, so a one-off starting-balance entry permanently dominated the bar green regardless of what was actually happening — reported as not useful after seeing it in practice. Both now compute from the same cycle-filtered set the log uses; balance is the only figure left all-time. Cache bumped to `?v=145`
- **Track bar removed entirely, same day.** Cycle-scoping it wasn't enough — still not wanted. The box is now just the balance plus the cycle-scoped "Added X · Used Y this cycle" line, no bar at all. Cache bumped to `?v=146`
- **Box height matched without a bar, same day.** Removing the track left the box shorter than Budget and Cards again. Fixed with padding instead of content — `21px 14px` instead of the flat `14px` every other Finance box uses, measured to land on the same 92px height. No filler row, no bar, just more breathing room. Cache bumped to `?v=147`
- **Savings recoloured, card meta reordered, same day.** `SAVINGS_COLOR` changed from teal (`#14b8a6`) to a lighter green (`#4ade80`) than ILA's card, same family rather than a clash. Unrelated but requested together: the card tile's meta row now reads Available, then Limit (was Limit, then Available). Cache bumped to `?v=148`
- **Card tile corrected same day — the meta-row swap above wasn't what was asked for.** The actual request: Available becomes the bold headline figure (replacing the raw balance), Limit and Used sit small underneath it, Limit on the left. Two follow-up passes to land there — Available first went into the headline with Used/Limit below it, then Limit and Used swapped back so Limit reads first. Final state: headline = Available, meta row = Limit then Used. Cache bumped to `?v=149`
